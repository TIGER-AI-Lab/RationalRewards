        ...
        def _wrap(batch):
            ...
            for i in range(n):
                ...
                try:
                    ...
                    if features:
                        missing_keys = [k for k in features if k not in processed]
                        if missing_keys:
                            msg = (
                                f"Schema mismatch.\n"
                                f"  Missing keys: {missing_keys}\n"
                                f"  Available keys in processed sample: {list(processed.keys())}\n"
                                f"  Required keys in schema: {list(features.keys())}"
                            )
                    ...
                    input_data = {k: batch[k][i] for k in batch}
                    print(f"Skipping bad sample index {i}: {e}")
                    print(f"Input data content: {json.dumps(input_data, ensure_ascii=False)}")

                    rec = {
                        "time": datetime.now().isoformat(timespec="seconds"),
                        "error": str(e),
                        "sample_index_in_batch": i,
                        "data": input_data,  # the original item
                    }
            ...
            #         out_batch[col] = [[] for _ in range(kept)]

            # return {col: out_batch[col] for col in features}
