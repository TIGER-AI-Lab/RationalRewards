...
class AlpacaDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.dataset_attr.kto_tag and isinstance(example[self.dataset_attr.kto_tag], bool):  # kto example
            ...
            response = []
        if self.dataset_attr.mask_history:
            print(f">>>>> dataset {self.dataset_attr.dataset_name} mask_history: {self.dataset_attr.mask_history}")
        output = {
            "_prompt": prompt,
            "_response": response,
            ...
            "_mask_history": self.dataset_attr.mask_history,
        }
        return output
...
def align_dataset(
    ...
    print(f">>>>> dataset {dataset_attr.dataset_name} mask_history: {dataset_attr.mask_history}")
