...
## Refined Request: [The improved, more specific user request for editing like a standard user instruction]"""
    ...
    async def evaluate_image(self, image_bytes, prompt, ref_image_bytes=None, requirement=""):
        # Convert bytes to base64
        image_b64 = base64.b64encode(image_bytes).decode()
        ref_image_b64 = base64.b64encode(ref_image_bytes).decode() if ref_image_bytes else ""

        # Create message
        messages = self._create_scoring_message(prompt, ref_image_b64, image_b64, requirement)

        # Generate evaluation
        response = await self.generate(messages)
        score = self._parse_scoring_response(response)
        return score


def dump_sample_subset(images, prompts, ref_images, all_scores, config, iteridx, intermediate):
    """Dump a subset of samples based on reward scores to analyze training progress."""
    import json
    import os
    from collections import defaultdict

    # Convert inputs to list of PIL images if needed
    if isinstance(images, torch.Tensor):
        # Handle single tensor case (shouldn't happen with new accumulation logic)
        images = (images * 255).round().clamp(0, 255).to(torch.uint8).cpu().numpy()
        # Assume (n, h, w, c) format
        pil_images = []
        for i in range(images.shape[0]):
            img_array = images[i]  # (h, w, c)
            # Ensure it's RGB
            ...
            pil_images.append(Image.fromarray(img_array.astype(np.uint8)))
        images = pil_images
    elif isinstance(images, list):
        # Ensure all elements in the list are PIL images
        processed_images = []
        for img in images:
            if isinstance(img, torch.Tensor):
                ...
                if img_np.ndim == 3:  # (h, w, c)
                    processed_images.append(Image.fromarray(img_np.astype(np.uint8)))
                else:
                ...
                processed_images.append(Image.fromarray(np.array(img).astype(np.uint8)))
        images = processed_images

    # ref_images should already be a list of PIL images, no conversion needed
    ...
    for i, prompt in enumerate(prompts):
        prompt_groups[prompt].append({
            'image': images[i]  ,
            'ref_image': ref_images[i] if i < len(ref_images) else ref_images[0],
            'score': all_scores[i],
            'index': i
        })
    ...
    for prompt, samples in prompt_groups.items():
        ...
        group_means[prompt] = np.mean(scores)

    # Find groups with min, max, and median means
    if len(group_means) == 0:
        return

    sorted_groups = sorted(group_means.items(), key=lambda x: x[1])
    means = [mean for _, mean in sorted_groups]

    min_mean_prompt = sorted_groups[0][0]
    max_mean_prompt = sorted_groups[-1][0]

    # Find median group
    median_idx = len(means) // 2
    median_mean_prompt = sorted_groups[median_idx][0]

    selected_groups = {
        'low_mean': min_mean_prompt,
        'high_mean': max_mean_prompt,
        'median_mean': median_mean_prompt
    }

    # Create output directory
    output_dir = os.path.join(config.save_dir, intermediate, f"iter_{iteridx}_samplings")
    os.makedirs(output_dir, exist_ok=True)

    # Save selected groups
    for group_name, prompt in selected_groups.items():
        group_dir = os.path.join(output_dir, group_name)
        os.makedirs(group_dir, exist_ok=True)

        samples = prompt_groups[prompt]

        # Save reference image (same for all samples in group)
        ref_image = samples[0]['ref_image']
        if hasattr(ref_image, 'save'):
            # PIL Image
            ref_image.save(os.path.join(group_dir, "source.png"))
        else:
            # numpy array
            ref_pil = Image.fromarray(ref_image.astype(np.uint8))
            ref_pil.save(os.path.join(group_dir, "source.png"))

        # Save generated images and collect scores
        image_scores = []
        for i, sample in enumerate(samples):
            # Save generated image
            if hasattr(sample['image'], 'save'):
                sample['image'].save(os.path.join(group_dir, f"generated_{i}.png"))
            else:
                gen_pil = Image.fromarray(sample['image'].astype(np.uint8))
                gen_pil.save(os.path.join(group_dir, f"generated_{i}.png"))

            image_scores.append({
                'image_filename': f"generated_{i}.png",
                'score': float(sample['score'])
            })

        # Save JSON with prompt and scores
        json_data = {
            'prompt': prompt,
            'images': image_scores,
            'group_mean_score': float(group_means[prompt])
        }

        with open(os.path.join(group_dir, "metadata.json"), 'w') as f:
            json.dump(json_data, f, indent=2)
            print(f"written {os.path.join(group_dir, 'metadata.json')}")


def direct_reward_fn(images, prompts, prompt_metadata, ref_images, only_strict=True, batch_size=32):
    # Handle inputs similar to flow_grpo.rewards.py
    if isinstance(images, torch.Tensor):
        # Convert tensor images to numpy: (N, C, H, W) -> (N, H, W, C)
        images = (images * 255).round().clamp(0, 255).to(torch.uint8).cpu().numpy()
        images = images.transpose(0, 2, 3, 1)  # NCHW -> NHWC
        images_batched = np.array_split(images, np.ceil(len(images) / batch_size))

    if not isinstance(ref_images, torch.Tensor):
        # ref_images is already a list of PIL images, convert to numpy arrays for batching
        ref_images_ = np.array([np.array(img) for img in ref_images])
        ref_images_batched = np.array_split(ref_images_, np.ceil(len(ref_images) / batch_size))
    # Split prompts and metadata into batches too
    prompts_batched = np.array_split(prompts, np.ceil(len(prompts) / batch_size))
    metadata_batched = np.array_split(prompt_metadata, np.ceil(len(prompt_metadata) / batch_size))
    ...
    all_scores = []
    client = QwenVLInferenceHTTP()
    print(">>>>>>>>>>> Checking connection...")
    # client.check_connection()
    print(f"split {len(images)} total images into {len(images_batched)} batches")
    # print(">>>>>>>>>>> Connection successful")
    for image_batch, ref_image_batch, prompt_batch, metadata_batch in zip(
        images_batched, ref_images_batched, prompts_batched, metadata_batched
    ):
        # Convert image batches to bytes
        image_bytes_list = [pil_to_bytes(img) for img in image_batch]
        ref_image_bytes_list = [pil_to_bytes(img) for img in ref_image_batch]

        requirements = [m.get("requirement", "") for m in metadata_batch]
        print(f"running batch eval for {len(image_bytes_list)} images")
        async def run_batch_eval():
            
            tasks = []
            for img_b, p, ref_b, req in zip(image_bytes_list, prompt_batch, ref_image_bytes_list, requirements):
                tasks.append(client.evaluate_image(img_b, p, ref_b, req))
            return await asyncio.gather(*tasks)

        batch_scores = asyncio.run(run_batch_eval())
        all_scores.extend(batch_scores)

    # Note: Dumping is now handled after all batches are collected

    # Return format: rewards (dict), reward_metadata
    rewards = {"avg": all_scores}
    reward_metadata = [None] * len(all_scores)
    # keylist = ['text_faithfulness', 'physical_quality']
    ...
    #             invalid = True 
    #             break 
    #     num_invalid += float(invalid) 
    # print(f">>> [debug] rate of invalid scores: {num_invalid / len(all_scores)}")
...
class PromptImageDataset(Dataset):
    def __init__(self, dataset, filepath, resolution=512, split="train"):
        self.dataset = dataset
        self.resolution = resolution
        # self.file_path = os.path.join(dataset, f"{split}_metadata.jsonl")
        self.file_path = filepath.format(split=split)
    ...
    def __getitem__(self, idx):
        ...
        image_path = self.metadatas[idx]["image_path"]
...
def eval_fn(
    ...
    if is_main_process(rank):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            #     step=global_step,
            # )
            print(f"step {global_step} logstats: {logstats}")
...
def main(_):
    ...
    print(f">>>>>>> loading pipeline from {config.pretrained.model}")
    ...
    print(f">>>>>>> preparing text encoder")
    prepare_fsdp_model(
    ...
    print(f">>>>>>> preparing text encoder 2")
    prepare_fsdp_model(
    ...
    train_dataset = PromptImageDataset(
        config.dataset, config.datapath, config.resolution, "train"
    )
    test_dataset = PromptImageDataset(config.dataset, config.datapath, config.resolution, "test")
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        pipeline.transformer.eval()
        samples_data_list = []
        # For dumping: accumulate data across all batches (only when dumping is enabled)
        should_dump = is_main_process(rank) # epoch % 5 == 0 and is_main_process(rank)
        all_dumping_images = [] if should_dump else None
        all_dumping_prompts = [] if should_dump else None
        all_dumping_ref_images = [] if should_dump else None

        for i in tqdm(
            range(config.sample.num_batches_per_epoch),
            desc=f"Epoch {epoch}/{config.num_epochs} batch sampling",
            disable=not is_main_process(rank),
            position=0,
        ):
            transformer_ddp.module.set_adapter("default")
            ...
            # Accumulate data for potential dumping
            if should_dump:
                # Convert tensor images to list of PIL images
                if isinstance(images, torch.Tensor):
                    images = (images * 255).round().clamp(0, 255).to(torch.uint8).cpu().numpy()
                    images_np = images.transpose(0, 2, 3, 1)  # NCHW -> NHWC
                    # images_np = (images * 255).round().clamp(0, 255).to(torch.uint8).cpu().numpy()
                    # Assume (n, h, w, c) format
                    pil_images = []
                    for i in range(images_np.shape[0]):
                        img_array = images_np[i]  # (h, w, c)
                        pil_images.append(Image.fromarray(img_array.astype(np.uint8)))
                    all_dumping_images.extend(pil_images)
                else:
                    all_dumping_images.extend(images)
                all_dumping_prompts.extend(prompts)
                all_dumping_ref_images.extend(ref_images)
        ...
        # Dump samples if requested (every 10 epochs)
        if should_dump:
            # Collect all scores from all batches
            all_scores = []
            for sample_item in samples_data_list:
                batch_scores = sample_item["rewards"]["avg"].cpu().numpy()
                all_scores.extend(batch_scores)
        
            dump_sample_subset(all_dumping_images, all_dumping_prompts, all_dumping_ref_images, all_scores, config, global_step, "intermediate")
        ...
        if is_main_process(rank):  # logging
            ...
            print(f"step {global_step} logstats1: {logstats}")
        ...
        if config.per_prompt_stat_tracking:
            ...
            if is_main_process(rank):
                ...
                print(f"step {global_step} logstats2: {logstats}")
            stat_tracker.clear()
        else:
                ...
                for j_idx, j_timestep_orig_idx in tqdm(
                    ...
                    if current_accumulated_steps % effective_grad_accum_steps == 0:
                        ...
                        if is_main_process(rank):
                            ...
                            print(f"step {global_step} logstats3: {json.dumps(logstats, indent=4)}")
