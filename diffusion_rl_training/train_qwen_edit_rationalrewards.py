...
class QwenVLInferenceHTTP:
    def __init__(self,
        ...
        self.generate_endpoint = "{}/v1/chat/completions".format(self.base_url)
        self.health_endpoint = "{}/v1/models".format(self.base_url)
    ...
    def check_connection(self):
        try:
            ...
            print("⚠ Warning: Could not connect to server: {}".format(e))
            return False
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        print("HTTP Error {}: {}".format(response.status, error_text))
                        return None
        except Exception as e:
            print("Error in generate: {}".format(e))
            return None
...
## Refined Request: [The improved, more specific user request for editing like a standard user instruction]"""
    ...
    def _parse_scoring_response(self, response):
        ...
        try:
            ...
            if scores:
                return {k: (v-1.)/3 for k,v in parsed_result.items() if v is not None and not isinstance(v, str)}
...
def direct_reward_fn(images, prompts, prompt_metadata, ref_images, only_strict=True, batch_size=32):
    ...
    print("split {} total images into {} batches".format(len(images), len(images_batched)))
    ...
    for image_batch, ref_image_batch, prompt_batch, metadata_batch in zip(
        ...
        print("running batch eval for {} images".format(len(image_bytes_list)))
    ...
    rewards = {"avg": [np.mean(list(entry.values())) if len(entry) > 0 else 0.0 for entry in all_scores]}
...
class PromptImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        item["prompt_with_image_path"] = "{}_{}".format(self.prompts[idx], image_path)
...
class DistributedKRepeatSampler(Sampler):
    def __init__(
        ...
        ), "k can not div n*b, k{}-num_replicas{}-batch_size{}".format(k, num_replicas, batch_size)
    ...
    if is_main_process(rank):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for idx in range(num_samples_to_log):
                ...
                pil.save(os.path.join(tmpdir, "{}.jpg".format(idx)))
            ...
            wandb.log(
                {
                    "eval_images": [
                        wandb.Image(
                            os.path.join(tmpdir, "{}.jpg".format(idx)),
                            caption="{:.1000} | ".format(prompt)
                            + " | ".join(
                                "{}: {:.2f}".format(k, v) for k, v in reward.items() if v != -10
                            ),
                        )
...
def save_ckpt(
    ...
    if is_main_process(rank):
        save_root = os.path.join(save_dir, "checkpoints", "checkpoint-{}".format(global_step))
        ...
        if config.train.ema and ema is not None:
            ema.copy_temp_to(transformer_trainable_parameters)
        logger.info("Saved checkpoint to {}".format(save_root))
...
def main(_):
    ...
    reward_fn = direct_reward_fn
    eval_reward_fn = direct_reward_fn
