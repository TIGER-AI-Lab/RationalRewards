...
def _get_config(base_model="kontext", n_gpus=1, gradient_step_per_epoch=1, reward_fn={}, name=""):
    ...
    config.dataset = "rl"
    # config.datapath = "<path/to/data>/generative_pointwise_rl_{split}.jsonl"
    config.datapath = "<path/to/data>/{split}.jsonl"
    print(f"Using datapath: {config.datapath}")
    config.pretrained.model = "<path/to/pretrained/model>"
    config.sample.num_steps = 15 # 6
    ...
    while True:
        ...
        print(f"{num_groups},{config.sample.num_image_per_prompt} % {n_gpus},{bsz}")
        if (
    ...
    config.save_dir = f"<path/to/logs>/{base_model}/{name}"
