...
# Batch evaluation script for image editing models
...
check_server_ready() {
    local max_attempts=120  # Maximum attempts (60 * 5s = 5 minutes)
    local attempt=1
...
for checkpoint in "${CHECKPOINT_ARRAY[@]}"; do
    ...
    eval_setting="${model_basename}_s${checkpoint}_edit"
    ...
    # python3 ./rm_inference/inference_genaibench_imggen.py --data-file "${GENAIBENCH_IMGGEN_DATA}" --evalsetting "${eval_setting}_gen"
    python3 ./rm_inference/inference_genaibench_imgedit.py --data-file "${GENAIBENCH_IMGEDIT_DATA}" --evalsetting "${eval_setting}"
    sleep 30
    python3 ./rm_inference/inference_mmrb2_imgedit.py --data-file "${MMRB2_IMGEDIT_DATA}" --evalsetting "${eval_setting}"
    sleep 30
    # python3 ./rm_inference/inference_mmrb2_imggen.py --data-file "${MMRB2_IMGGEN_DATA}" --evalsetting "${eval_setting}_gen"
    python3 ./rm_inference/inference_editreward_imgedit.py --data-file "${EDITREWARD_IMGEDIT_DATA}" --evalsetting "${eval_setting}"
